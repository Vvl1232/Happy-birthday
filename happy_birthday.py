...
<head>
    ...
    <style>
        * {
        ...
        /* THEME: powder sky blue background (requested) */
        body {
        ...
        .welcome-screen {
            ...
            z-index: 10000;
            background: linear-gradient(135deg, rgba(223,246,255,0.06) 0%, rgba(207,239,255,0.06) 50%, rgba(233,248,255,0.06) 100%);
            animation: welcomeSequence 1.05s ease-in-out forwards; /* shortened per your earlier requests */
        }
        ...
        /* Peacock animation container (kept but peacock later removed/placed among emojis if you want) */
        .peacock-container {
            position: fixed;
            top: 0;
            left: 0;
            ...
            align-items: center;
            z-index: 9999;
            background: linear-gradient(135deg, rgba(223,246,255,0.04) 0%, rgba(207,239,255,0.04) 50%, rgba(233,248,255,0.04) 100%);
            animation: peacockSequence 3s ease-in-out forwards; /* shortened */
        }
        ...
        .peacock {
            ...
            animation: peacockFlyStop 3s ease-out forwards;
        }
        
        @keyframes peacockFlyStop {
            0% { transform: translateX(-120%) scale(1.2); opacity: 0; }
            10% { transform: translateX(-120%) scale(1.2); opacity: 0; }
            11% { transform: translateX(-10%) scale(1.2); opacity: 1; }
            70% { transform: translateX(5%) scale(1.1); opacity: 1; }
        ...
        .countdown {
            ...
            font-weight: bold;
            color: #ff6b6b;
            text-shadow: 0 0 20px rgba(255, 107, 107, 0.6), 0 0 40px rgba(255, 107, 107, 0.4);
            z-index: 10001;
        ...
        .birthday-content {
            ...
            animation: contentSequence 1.5s ease-in forwards;
            max-width: 85%;
            max-height: 85vh;
            overflow-y: auto;
        }
        ...
        @keyframes contentSequence {
            0% { opacity: 0; visibility: hidden; transform: translate(-50%, -50%) scale(0.95); }
            99% { opacity: 0; visibility: hidden; transform: translate(-50%, -50%) scale(0.95); }
        ...
        .firework {
            font-size: 3.4em;
            animation: firework 1.6s ease-out infinite;
        }
...
<body>
    ...
    <!-- Flute background music (WebAudio) -->
    <script>
    (function(){
      // Simple flute-like Happy Birthday loop. Attempts autoplay; resumes on first gesture.
      const notes = [
        "G4","G4","A4","G4","C5","B4",
        "G4","G4","A4","G4","D5","C5",
      ...
      function ensure(){
        ...
        master.gain.value = 0.45; // moderate volume for clarity
      ...
      function startLoop(){
        ...
        audioCtx.resume().catch(()=>{}).finally(()=>{
          ...
          loopHandle = setInterval(()=> scheduleOnce(audioCtx.currentTime + 0.04), Math.max(80, (total*1000) - 30));
          playing = true;
        });
      }
      ...
      // attempt autoplay; if blocked, start on gesture
      ...
      window.addEventListener('pointerdown', gestureStarter, {passive:true});
      window.addEventListener('keydown', gestureStarter, {passive:true});
      window.addEventListener('touchstart', gestureStarter, {passive:true});

      // subtle click chime on interaction if audio running
    ...
    <script>
        // Countdown animation with smooth transitions (shortened timeline to fit new welcome timing)
        ...
        // Start countdown soon after welcome (short timeline)
        showNumber('3', 600); // Show 3 at 0.6s
        showNumber('2', 750); // Show 2 at 0.75s
        showNumber('1', 900); // Show 1 at 0.9s
        
        // Clear countdown shortly after
        setTimeout(() => {
